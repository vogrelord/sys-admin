...
jQuery.fn.wymeditor = function(options) {
  ...
  options = jQuery.extend({
    ...
    dialogLinkHtml: "<div class='wym_dialog wym_dialog_link'>"
      + "<h4 class='wym_dialog_title'>{Link}</h4>"
      + "<form>"
      + "<div class='row'>"
      + "<label>{URL}</label>"
    ...
    dialogImageHtml: "<div class='wym_dialog wym_dialog_image'>"
      + "<h4 class='wym_dialog_title'>{Image}</h4>"
      + "<form>"
      + "<div class='row'>"
      + "<label>{URL}</label>"
    ...
    dialogTableHtml: "<div class='wym_dialog wym_dialog_table'>"
      + "<h4 class='wym_dialog_title'>{Table}</h4>"
      + "<form>"
      + "<div class='row'>"
      + "<label>{Caption}</label>"
    ...
    dialogPreviewHtml: "<div class='wym_dialog'>"
      + "<h4 class='wym_dialog_title'>Preview</h4>"
      + "<div class='wym_dialog_preview'></div>"
      + "<input class='wym_cancel' type='button' value='{Cancel}' />"
      + "</div>",
...
WYMeditor.editor.prototype.dialog = function( dialogType, bodyHtml ) {
  ...
  switch( dialogType ) {
    case WYMeditor.DIALOG_LINK:
      okFunction = function() {
        ...
        var sTitle = jQuery( wym._options.titleSelector ).val();
        wym._exec( WYMeditor.CREATE_LINK, sStamp );
        console.log( sUrl, sTitle, sStamp, selected );
        $( 'a', wym._doc.body ).filter( "a[href*='" + sStamp + "']" )
          .attr( WYMeditor.HREF, sUrl )
          .attr( WYMeditor.TITLE, sTitle );
        wymRemoveDialog();
    ...
    case WYMeditor.DIALOG_IMAGE:
      okFunction = function() {
        ...
        var sTitle = jQuery( wym._options.titleSelector ).val();
        var sAlt = jQuery( wym._options.altSelector ).val();
  ...
  var wymBox = jQuery( '.wym_box_' + wym._index );
  wymBox.prepend( dialogHtml );
  $( '.wym_submit' ).bind( 'click', function(){okFunction();} );
  $( '.wym_cancel' ).bind( 'click', function(){wymRemoveDialog();} );

  var dialog = jQuery( '.wym_dialog' );
  dialog.find( ':input:first' ).focus();
  var dialogTop = ( ( wymBox.height() / 2 ) - ( dialog.height() / 2 ) ) + dialog.position().top;
  var dialogLeft = ( wymBox.width() / 2 ) - ( dialog.width() / 2 ) + dialog.position().left;
  ...
  jQuery( wym._options.dialogPreviewSelector ).html( wym.xhtml() );
};
